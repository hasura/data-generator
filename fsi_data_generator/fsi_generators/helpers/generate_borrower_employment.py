...
def generate_random_borrower_employment(id_fields: Dict[str, Any], dg) -> Dict[str, Any]:
    ...
    end_date = None
    if not is_current:
        months_after_start = random.randint(6, years_ago * 12 - 1)  # At least 6 months of employment
    ...
    # Get a random address ID for the employer
    enterprise_address_id = get_random_address_id(conn, is_business=True)
...
def get_random_address_id(conn, is_business: bool = False) -> Optional[int]:
    ...
    try:
        ...
        if is_business:
            # Try to get a business address
            cursor.execute("""
                SELECT ea.enterprise_address_id 
                FROM enterprise.entity_addresses ea
                WHERE ea.entity_type = 'business'
                ORDER BY RANDOM()
                LIMIT 1
            """)
            ...
            if not result:
                # If no business addresses, try getting any address
                cursor.execute("""
        ...
        print(f"Error fetching random address ID: {error}")
        return None
