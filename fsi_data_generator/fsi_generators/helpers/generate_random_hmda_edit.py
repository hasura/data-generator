...
def generate_edit_description(edit_type: str, edit_code: str, hmda_info: Optional[Dict[str, Any]], dg: DataGenerator) -> str:
    """
    ...
    if edit_type == "syntactical":
        try:
            alt_syntactical_messages = generate_unique_json_array(dbml_string=dg.dbml, fully_qualified_column_name='description of a syntactical HMDA error', count=50, cache_key='syntactical_hmda_edit_error')
        except Exception as e:
            print(f"Warning: Couldn't generate alt_syntactical_messages: {e}")
    ...
    # Validity errors relate to invalid values or combinations
        ...
        try:
            alt_validity_messages = generate_unique_json_array(dbml_string=dg.dbml, fully_qualified_column_name='description of a validity HMDA error', count=50, cache_key='validity_hmda_edit_error')
        except Exception as e:
            print(f"Warning: Couldn't generate alt_syntactical_messages: {e}")
...
def generate_resolution_notes(status: str, _edit_type: str, edit_code: str, dg: DataGenerator) -> str:
    ...
    if status == "verified":
        ...
        try:
            alt_verified_notes = generate_unique_json_array(dbml_string=dg.dbml, fully_qualified_column_name='Notes for a verified HMDA edit', count=50, cache_key='verified_hmda_edit_notes')
        except Exception as e:
            print(f"Warning: Couldn't generate unique notes: {e}")
            alt_verified_notes = []
        verified_notes = [
        ...
        # Try to get custom notes from DataGenerator, but handle possible failure
        try:
            alt_corrected_notes = generate_unique_json_array(dbml_string=dg.dbml, fully_qualified_column_name='Notes for a corrected HMDA edit', count=50, cache_key='corrected_hmda_edit_notes')
        except Exception as e:
            print(f"Warning: Couldn't generate unique notes: {e}")
            alt_corrected_notes = []
        corrected_notes = [
