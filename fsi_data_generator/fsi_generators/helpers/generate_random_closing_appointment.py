...
def generate_random_closing_appointment(id_fields: Dict[str, Any], dg) -> Dict[str, Any]:
    ...
    # Get random address ID for the closing location
    location_address_id = get_random_address_id(conn, is_business=True)
...
def get_random_address_id(conn, is_business: bool = False) -> Optional[int]:
    ...
    try:
        ...
        if is_business:
            # Try to get a business address (like a title company or bank)
            cursor.execute("""
