...
if __name__ == "__main__":
    # Example of a loan product
    sample_product = {
        "product_name": "Premier Advantage Fixed Mortgage",
        "product_code": "PAF30",
        "loan_type": "conventional",
        "interest_rate_type": "fixed",
        "base_interest_rate": 3.875,
        "min_term_months": 180,  # 15 years
    ...
    sample_application = {
        ...
        "status": "approved",
        "creation_date_time": "2023-01-15 09:30:45.123456",
        "loan_purpose": "primary residence",
        "requested_loan_amount": 320000.00,
        "requested_loan_term_months": 360,
        "estimated_property_value": 400000.00,
        "estimated_credit_score": 720,
        "referral_source": "real estate agent"
    }

    # Generate a mortgage using both product constraints and application details
    mortgage = generate_random_mortgage(sample_product, sample_application)

    # Print basic details
    print(f"Loan Product: {mortgage.get('loan_product', 'N/A')}")
    print(f"Loan Type: {mortgage.get('loan_type', 'N/A')}")
    print(f"Loan Amount: ${mortgage['loan_amount']:,.2f}")
    print(f"Interest Rate: {mortgage['interest_rate']}%")
    print(f"Term: {mortgage['loan_term_months']} months")
    print(f"Down Payment: ${mortgage['down_payment']:,.2f} ({mortgage['down_payment_percentage']}%)")

    # Print dates
    print(f"Origination Date: {mortgage['origination_date']}")
    print(f"First Payment Date: {mortgage['first_payment_date']}")
    print(f"Maturity Date: {mortgage['maturity_date']}")

    # Print payment details
    print(f"Monthly Principal & Interest: ${mortgage['monthly_payment']:,.2f}")
    print(f"Monthly Escrow: ${mortgage['escrow_amount']:,.2f}")

    # Print PMI if applicable
    if mortgage['private_mortgage_insurance']:
        print(f"PMI Required: Yes (Rate: {mortgage['pmi_rate']}%)")
        monthly_pmi = (mortgage['loan_amount'] * (mortgage['pmi_rate'] / 100)) / 12
        print(f"Monthly PMI: ${monthly_pmi:,.2f}")
    else:
        print("PMI Required: No")

    # Print closing details
    print(f"Closing Costs: ${mortgage['closing_costs']:,.2f}")
