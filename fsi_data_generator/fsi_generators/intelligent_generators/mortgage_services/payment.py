...
def generate_random_payment(id_fields: Dict[str, Any], dg) -> Dict[str, Any]:
    ...
    next_payment_due_date = next_payment_due_date.date() if hasattr(next_payment_due_date,
                                                                    'date') else next_payment_due_date
    ...
    if previous_payments:
        ...
        earliest_payment_date = previous_payments[-1]["payment_date"] if len(previous_payments) > 1 else \
            previous_payments[0]["payment_date"]

        # Ensure earliest_payment_date is in date format
        earliest_payment_date = earliest_payment_date.date() if hasattr(earliest_payment_date,
                                                                        'date') else earliest_payment_date
    ...
    payment_date_obj = payment_date.date() if hasattr(payment_date, 'date') else payment_date
    loan_origination_date_obj = loan_origination_date.date() if hasattr(loan_origination_date,
                                                                        'date') else loan_origination_date
    ...
    regular_payment_amount = float(servicing_account_info.get("payment_amount", 1500.00) or 1500.00)
    ...
    if "current_principal_balance" in servicing_account_info and servicing_account_info[
        "current_principal_balance"] is not None:
        loan_balance = float(servicing_account_info["current_principal_balance"] or 0.0)
        interest_rate = float(servicing_account_info.get("interest_rate", 4.5) or 4.5)

        # Calculate monthly interest - ensure values are not None
        if loan_balance > 0 and interest_rate > 0:
    ...
    if payment_type == PaymentType.ESCROW:
        ...
        if "monthly_escrow_amount" in servicing_account_info and servicing_account_info[
            ...
            escrow_amount = float(servicing_account_info["monthly_escrow_amount"] or 0.0)
...
def _get_previous_payments(servicing_account_id: int, conn) -> list:
    ...
    try:
        ...
        logger.error(f"Error fetching previous payments: {error}")
        return []
