...
def get_party_info(party_id: Optional[int], conn) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Get associated addresses - updated to use party_entity_addresses table
        cursor.execute("""
            SELECT pea.enterprise_address_id
            FROM enterprise.party_entity_addresses pea
            WHERE pea.enterprise_party_id = %s
            ORDER BY pea.enterprise_party_entity_address_id
        """, (party_id,))
        ...
        logger.error(f"Error fetching party information: {error}")
        return None
...
def get_or_create_address(conn) -> Optional[int]:
    ...
    try:
        ...
        logger.error(f"Error fetching address ID: {error}")
        return None
