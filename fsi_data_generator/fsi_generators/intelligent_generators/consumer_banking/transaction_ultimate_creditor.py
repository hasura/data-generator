...
def generate_random_transaction_ultimate_creditor(id_fields: Dict[str, Any], dg: DataGenerator) -> Dict[str, Any]:
    """
    Generate a random transaction ultimate creditor with plausible values.
    May raise SkipRowGenerationError if the transaction type doesn't typically have an ultimate creditor.
    ...
    Returns:
        Dictionary containing randomly generated transaction ultimate creditor data
    """
    ...
    try:
        ...
        # Check if there's a creditor agent/account already for this transaction
        cursor.execute("""
            SELECT COUNT(*) as has_creditor
            FROM consumer_banking.transaction_creditor_agents
            WHERE consumer_banking_transaction_id = %s
        """, (id_fields['consumer_banking_transaction_id'],))

        has_creditor_agent = cursor.fetchone().get('has_creditor', 0) > 0

        cursor.execute("""
            SELECT COUNT(*) as has_creditor_account
            FROM consumer_banking.transaction_creditor_accounts
            WHERE consumer_banking_transaction_id = %s
        """, (id_fields['consumer_banking_transaction_id'],))

        has_creditor_account = cursor.fetchone().get('has_creditor_account', 0) > 0

        # Ultimate creditors are only used in specific transaction scenarios:
        ...
        # Skip if no creditor agent/account and not a transaction type that would have an ultimate creditor
        if not (has_creditor_agent or has_creditor_account):
            # 90% chance to skip if there's no creditor agent or account
            if random.random() < 0.9:
                raise SkipRowGenerationError

        # Ultimate creditors are more likely for certain transaction types
        ultimate_creditor_types = [
        ...
        if transaction_type_enum not in ultimate_creditor_types:
            # 80% chance to skip for non-typical transaction types
            if random.random() < 0.8:
                raise SkipRowGenerationError

        # Look for keywords in description suggesting an ultimate creditor
        description_lower = None
        if description:
            description_lower = description.lower()
            ultimate_creditor_keywords = [
                'via', 'through', 'for', 'attn:', 'beneficiary', 'final', 'recipient',
                'c/o', 'care of', 'attention', 'ref:', 'reference', 'ultimate'
            ]
            has_keyword = any(keyword in description_lower for keyword in ultimate_creditor_keywords)

            if not has_keyword:
                # 70% chance to skip if no keyword suggesting an ultimate creditor
                if random.random() < 0.7:
                    raise SkipRowGenerationError

        # Generate ultimate creditor name
        # If there's description text, try to extract a creditor name from it
        name = None
        if description:
            # Look for patterns like "via [NAME]" or "for [NAME]" in the description
            for prefix in ['via ', 'through ', 'for ', 'attn: ', 'beneficiary: ', 'c/o ', 'care of ']:
        ...
        if not name:
            # Decide if individual or business (60% business for ultimate creditors)
            if random.random() < 0.6:  # Business name
        ...
        if random.random() < 0.4:  # 40% chance of having identifier
            id_types = ["ACC", "ID", "REF", "CUS", "BEN", "SUP", "PAY", "REC"]
        ...
        # Generate LEI for organizations (rarely used for ultimate creditors)
        lei = None
        ...
        scheme_name = None
        if identification:
            # For ultimate creditors, scheme_name refers to the identification scheme for the beneficiary
            # Not to be confused with the financial institution identifier scheme
            schemes = [
                "Customer ID",
                "Account Reference",
        ...
        # Create the ultimate creditor dictionary
        ultimate_creditor = {
