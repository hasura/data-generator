...
WITH ins_depts AS (
    ...
    INSERT INTO enterprise.departments (department_name, location, operating_unit) VALUES ('Human Resources',
                                                                                           'HQ Bldg A',
                                                                                           'HR'::enterprise.operating_unit),
                                                                                          ('Retail Branch Operations',
                                                                                           'Multiple Branches',
                                                                                           'CONSUMER_BANKING'::enterprise.operating_unit),
                                                                                          ('IT Security',
                                                                                           'Tech Center B',
                                                                                           'IT'::enterprise.operating_unit)
     ...
     ins_associates AS (
         ...
         INSERT INTO enterprise.associates (first_name, last_name, email, job_title, status, relationship_type, salary,
                                            enterprise_department_id, hire_date, manager_id)
             -- Managers/Owners first (no manager assumed for them in this scope)
             SELECT 'Hannah',
                    'HRHead',
                    'hannah.hrhead@bank.com',
                    'HR Manager',
                    'ACTIVE'::enterprise.associate_status,
                    'EMPLOYEE'::enterprise.relationship_status,
                    95000.00,
                    dept.enterprise_department_id,
                    '2018-03-15'::date,
                    NULL::integer
             FROM ins_depts dept
             WHERE dept.department_name = 'Human Resources'
             UNION ALL
             SELECT 'Olivia',
                    'OpsLead',
                    'olivia.opslead@bank.com',
                    'Retail Ops Manager',
                    'ACTIVE'::enterprise.associate_status,
                    'EMPLOYEE'::enterprise.relationship_status,
                    98000.00,
                    ...
                    NULL::integer
             FROM ins_depts dept
             WHERE dept.department_name = 'Retail Branch Operations'
             UNION ALL
             SELECT 'Ian',
                    'ITAdmin',
                    'ian.itadmin@bank.com',
                    'IT Security Admin',
                    'ACTIVE'::enterprise.associate_status,
                    'EMPLOYEE'::enterprise.relationship_status,
                    90000.00,
                    ...
                    NULL::integer
             FROM ins_depts dept
             WHERE dept.department_name = 'IT Security'
             UNION ALL
             -- Tellers (managed by Olivia OpsLead)
             SELECT 'Alice',
                    'Example',
                    'alice.example@bank.com',
                    'Teller',
                    'ACTIVE'::enterprise.associate_status,
                    'EMPLOYEE'::enterprise.relationship_status,
                    55000.00,
                    dept.enterprise_department_id,
                    '2021-05-01'::date,
                    (SELECT sub.enterprise_associate_id
                     FROM enterprise.associates sub
                     WHERE sub.email = 'olivia.opslead@bank.com')
             FROM ins_depts dept
             WHERE dept.department_name = 'Retail Branch Operations'
             UNION ALL
             SELECT 'Bob',
     ...
     ins_apps AS (
         ...
         INSERT INTO app_mgmt.applications (app_mgmt_application_id, application_name, description,
                                            enterprise_department_id, application_owner_id, created_by_user_id,
                                            application_type, lifecycle_status, date_deployed)
             SELECT 'a1111111-a111-a111-a111-a11111111111'::UUID,
                    'HR Management Portal',
                    'Manages sensitive employee PII, salary, benefits. Owning Department: Human Resources. Data Sensitivity: High.',
                    dept.enterprise_department_id,
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'hannah.hrhead@bank.com') as application_owner_id,
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'ian.itadmin@bank.com')   as created_by_user_id,
             ...
             WHERE dept.department_name = 'Human Resources'
             UNION ALL
             SELECT 'a2222222-a222-a222-a222-a22222222222'::UUID,
                    'Teller Application',
                    ...
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'olivia.opslead@bank.com') as application_owner_id,
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'ian.itadmin@bank.com')    as created_by_user_id,
     ...
     ins_identities AS (
         ...
         INSERT INTO security.identities (security_identity_id, name, display_name, owner_id, service_account,
                                          environment, created, status, inactive, modified, synced,
     ...
     ins_resource AS (
         ...
         INSERT INTO security.resource_definitions (security_resource_id, resource_name, resource_type,
                                                    resource_identifier, application_id, description, created_at,
                                                    created_by_id)
             SELECT 'cccccccc-cccc-cccc-cccc-cccccccccccc'::UUID,
                    ...
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'ian.itadmin@bank.com')                               as created_by_id
             FROM ins_apps apps
             WHERE apps.application_name = 'HR Management Portal'
             UNION ALL
             SELECT 'c1c1c1c1-cccc-cccc-cccc-cccccccccccc'::UUID,
                    ...
                    (SELECT ia.enterprise_associate_id
                     ...
                     WHERE ia.email = 'ian.itadmin@bank.com')                                         as created_by_id
             FROM ins_apps apps
             WHERE apps.application_name = 'Teller Application'
     ...
     ins_entitlement AS (
         ...
         INSERT INTO security.enhanced_entitlements (security_entitlement_id, entitlement_name, display_name,
                                                     description, managing_application_id, status, created_at,
                                                     created_by_id)
             SELECT 'dddddddd-dddd-dddd-dddd-dddddddddddd'::UUID,
             ...
             WHERE apps.application_name = 'HR Management Portal'
             UNION ALL
             SELECT 'd1d1d1d1-dddd-dddd-dddd-dddddddddddd'::UUID,
             ...
             WHERE apps.application_name = 'Teller Application'
             UNION ALL
             SELECT 'd2d2d2d2-dddd-dddd-dddd-dddddddddddd'::UUID,
     ...
     ins_ent_res AS (
         -- 5. Insert Entitlement-Resource Links with correct column reference
         -- Replace just the entitlement resources section with this
         INSERT INTO security.entitlement_resources
             (security_entitlement_resource_id, security_entitlement_id, security_resource_id, permission_type,
              created_at, created_by_id)
             VALUES ('77777777-7777-7777-7777-777777777777'::UUID,
                     'dddddddd-dddd-dddd-dddd-dddddddddddd'::UUID,
                     'cccccccc-cccc-cccc-cccc-cccccccccccc'::UUID,
                     'READ'::security.permission_type,
                     CURRENT_TIMESTAMP,
                     (SELECT enterprise_associate_id FROM enterprise.associates WHERE email = 'ian.itadmin@bank.com')),

                    ('71717171-7777-7777-7777-777777777777'::UUID,
                     'd1d1d1d1-dddd-dddd-dddd-dddddddddddd'::UUID,
                     'c1c1c1c1-cccc-cccc-cccc-cccccccccccc'::UUID,
                     'WRITE'::security.permission_type,
                     CURRENT_TIMESTAMP,
                     (SELECT enterprise_associate_id FROM enterprise.associates WHERE email = 'ian.itadmin@bank.com')),

                    ('72727272-7777-7777-7777-777777777777'::UUID,
                     'd2d2d2d2-dddd-dddd-dddd-dddddddddddd'::UUID,
                     'c1c1c1c1-cccc-cccc-cccc-cccccccccccc'::UUID,
                     'ADMIN'::security.permission_type,
                     CURRENT_TIMESTAMP,
                     (SELECT enterprise_associate_id FROM enterprise.associates WHERE email = 'ian.itadmin@bank.com'))
     ...
     ins_roles AS (
         -- 6. Insert Roles
         INSERT INTO security.roles (security_role_id, role_name, display_name, description, managing_application_id,
                                     owner_id, status, created_at, created_by_id)
             SELECT 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee'::UUID,
                    ...
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'olivia.opslead@bank.com'),
                    'ACTIVE'::security.role_status,
                    CURRENT_TIMESTAMP,
                    (SELECT ia.enterprise_associate_id FROM ins_associates ia WHERE ia.email = 'ian.itadmin@bank.com')
             FROM ins_apps apps
             WHERE apps.application_name = 'Teller Application'
             UNION ALL
             SELECT 'ffffffff-ffff-ffff-ffff-ffffffffffff'::UUID,
                    'TELLER',
                    'Teller',
             ...
             WHERE apps.application_name = 'Teller Application'
             UNION ALL
             SELECT '1a2b3c4d-1111-4ace-1111-111111111111'::UUID,
                    'HR_MANAGER_ROLE',
                    'HR Manager',
                    ...
                    (SELECT ia.enterprise_associate_id
                     FROM ins_associates ia
                     WHERE ia.email = 'hannah.hrhead@bank.com'),
                    'ACTIVE'::security.role_status,
                    CURRENT_TIMESTAMP,
                    (SELECT ia.enterprise_associate_id FROM ins_associates ia WHERE ia.email = 'ian.itadmin@bank.com')
             FROM ins_apps apps
             WHERE apps.application_name = 'HR Management Portal'
             UNION ALL
             SELECT 'faceb001-1111-1111-1111-111111111111'::UUID,
                    'payment_initiator',
                    'Payment Initiator',
             ...
             WHERE apps.application_name = 'Teller Application'
             UNION ALL
             SELECT 'faceb002-2222-2222-2222-222222222222'::UUID,
                    'payment_approver',
                    'Payment Approver',
             ...
             WHERE apps.application_name = 'Teller Application'
             UNION ALL
             SELECT 'faceb003-3333-3333-3333-333333333333'::UUID,
                    'payment_processor',
                    'Payment Processor',
     ...
     ins_role_ent AS (
         ...
         INSERT INTO security.role_entitlements (security_role_entitlement_id, security_role_id,
                                                 security_entitlement_id, created_at, started_at, active, created_by_id)
             SELECT '88888888-8888-8888-8888-888888888888'::UUID,
                    r.security_role_id,
                    ent.security_entitlement_id,
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP,
                    true,
                    (SELECT ia.enterprise_associate_id FROM ins_associates ia WHERE ia.email = 'ian.itadmin@bank.com')
             FROM ins_roles r,
                  ins_entitlement ent
             WHERE r.role_name = 'LEAD_TELLER'
               AND ent.entitlement_name = 'read_associate_details'
             UNION ALL
             SELECT '8a8a8a8a-8888-8888-8888-888888888888'::UUID,
                    r.security_role_id,
                    ent.security_entitlement_id,
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP,
             ...
             FROM ins_roles r,
                  ins_entitlement ent
             WHERE r.role_name = 'HR_MANAGER_ROLE'
               AND ent.entitlement_name = 'read_associate_details'
             UNION ALL
             SELECT '8b8b8b8b-1111-1111-1111-111111111111'::UUID,
                    r.security_role_id,
                    ent.security_entitlement_id,
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP,
             ...
             FROM ins_roles r,
                  ins_entitlement ent
             WHERE r.role_name = 'payment_initiator'
               AND ent.entitlement_name = 'initiate_payment'
             UNION ALL
             SELECT '8c8c8c8c-2222-2222-2222-222222222222'::UUID,
                    r.security_role_id,
                    ent.security_entitlement_id,
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP,
             ...
             FROM ins_roles r,
                  ins_entitlement ent
             WHERE r.role_name = 'payment_approver'
               AND ent.entitlement_name = 'approve_payment'
             UNION ALL
             SELECT '8d8d8d8d-3333-3333-3333-333333333333'::UUID,
                    r.security_role_id,
                    ent.security_entitlement_id,
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP,
             ...
             FROM ins_roles r,
                  ins_entitlement ent
             WHERE r.role_name = 'payment_processor'
               AND ent.entitlement_name = 'initiate_payment' -- Still linked
             UNION ALL
             SELECT '8e8e8e8e-3333-3333-3333-333333333333'::UUID,
                    r.security_role_id,
                    ent.security_entitlement_id,
                    CURRENT_TIMESTAMP,
                    CURRENT_TIMESTAMP,
             ...
             FROM ins_roles r,
                  ins_entitlement ent
             WHERE r.role_name = 'payment_processor'
               AND ent.entitlement_name = 'approve_payment' -- Still linked
         -- ON CONFLICT (security_role_entitlement_id) DO NOTHING

     )
-- 8. Final Insert: Assign roles to identities
INSERT
INTO security.identity_roles (security_identity_role_id, security_identity_id, security_role_id, start_date, active,
                              assigned_by_id)
-- Assign Tellers to LEAD_TELLER (Excessive) or TELLER (Corrected)
SELECT CASE i.name
           WHEN 'aexample' THEN '9a11ce00-aaaa-aaaa-aaaa-aaaaaaaaaaaa'::UUID
           WHEN 'ccheck'
               THEN '9c3e11e0-cccc-cccc-cccc-cccccccccccc'::UUID -- Charlie gets LEAD_TELLER here... AND payment_processor below
...
SELECT '91a11a11-aaaa-aaaa-aaaa-aaaaaaaaaaaa'::UUID,
       i.security_identity_id,
       r.security_role_id,
       CURRENT_TIMESTAMP,
       true,
...
FROM ins_identities i,
     ins_roles r
WHERE i.name = 'hhrhead'
  AND r.role_name = 'HR_MANAGER_ROLE'

UNION ALL

-- Assign Charlie Check the payment_processor role (SoD Violation)
SELECT '9f0d0000-cccc-cccc-cccc-cccccccccccc'::UUID,
       i.security_identity_id,
       r.security_role_id,
       CURRENT_TIMESTAMP,
       true,
...
FROM ins_identities i,
     ins_roles r
WHERE i.name = 'ccheck'
  AND r.role_name = 'payment_processor' -- Use new role name

UNION ALL

-- Assign Fiona Fine the INITIATOR role (Contrast)
SELECT '9f104e00-ffff-ffff-ffff-ffffffffffff'::UUID,
       i.security_identity_id,
       r.security_role_id,
       CURRENT_TIMESTAMP,
       true,
...
FROM ins_identities i,
     ins_roles r
WHERE i.name = 'ffine'
  AND r.role_name = 'payment_initiator' -- Use new role name

UNION ALL

-- Assign Olivia OpsLead the APPROVER role (Contrast)
SELECT '901141a0-0000-0000-0000-000000000000'::UUID,
       i.security_identity_id,
       r.security_role_id,
       CURRENT_TIMESTAMP,
       true,
...
FROM ins_identities i,
     ins_roles r
WHERE i.name = 'oopslead'
  AND r.role_name = 'payment_approver';
-- Use new role name
-- No ON CONFLICT

-- Update the existing resource_details field for the read_associate_details entitlement
UPDATE security.entitlement_resources
SET resource_details = 'data_type:PII, access_level:restricted, context:employee_records'
WHERE security_entitlement_id = 'dddddddd-dddd-dddd-dddd-dddddddddddd'
AND security_resource_id = 'cccccccc-cccc-cccc-cccc-cccccccccccc';

-- Update resource_details for initiate_payment entitlement
UPDATE security.entitlement_resources
SET resource_details = 'transaction_type:payment_initiation, limit:standard, audit_level:high'
WHERE security_entitlement_id = 'd1d1d1d1-dddd-dddd-dddd-dddddddddddd'
AND security_resource_id = 'c1c1c1c1-cccc-cccc-cccc-cccccccccccc';

-- Update resource_details for approve_payment entitlement
UPDATE security.entitlement_resources
SET resource_details = 'approval_level:standard, dual_control:required, amount_limit:corporate_policy'
WHERE security_entitlement_id = 'd2d2d2d2-dddd-dddd-dddd-dddddddddddd'
AND security_resource_id = 'c1c1c1c1-cccc-cccc-cccc-cccccccccccc';
